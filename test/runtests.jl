...
@testset "ItemResponseTheory.jl Package Tests" begin
    ...
    # ==========================================================================
    # 2. TAM Design Matrix System (TAM architecture)
    # ==========================================================================
    @testset "TAM Design Matrix System" begin
        # Create a simple TAMDesign for 2 items, 2 categories, 1 dimension
        # A: (2 × 2 × 2)
        A = zeros(2, 2, 2)
        A[1, 2, 1] = 1.0
        A[2, 2, 2] = 1.0
        ...
        tam_design = TAMDesign(A, B, xsi)
        @test tam_design.I == 2
        @test tam_design.K == 2
        @test tam_design.D == 1
        
        # Test probability tracing
        theta_tam = reshape([-1.0, 1.0], :, 1)
        P = prob_trace(tam_design, theta_tam)
        @test size(P) == (2, 2, 2)
        @test all(sum(P, dims=3) .≈ 1.0)
    end
